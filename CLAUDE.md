...
- Data sources: **akshare** (CN indices/monetary, 外围 environment, 黄金 SGE) + FRED (US M2/CPI for gold) + Tavily (news/research) + multpl.com (Shiller PE/CAPE + 美债10Y 月度,HTML 爬虫,用于 ERP) + WGC fsapi (黄金 AISC 季度,JSON API)。**Zero yfinance dependency** (the former US pipeline and all US/yfinance code were removed in the cn-pivot refactor).
...
- 5 个数据源(akshare/FRED/Tavily/multpl/WGC),无 yfinance。`run.py` 对 eastmoney 域名设置 NO_PROXY(系统代理 SSL 劫持会破坏 CN 行情/历史/资金流)。
...
| `data/` | `base.py` / `cn_data.py` / `gold_data.py` / `valuation_data.py` | **Stateful SQLite data layer** (P1) — providers read index/macro series from here; DB-First refresh fast-path;`us_data.py` 已删,见 Data Layer above;`valuation_data.py` = `ValuationData.update_erp`(multpl CAPE+美债10Y → ERP 月度序列) |
| `datasources/` | `akshare.py` / `tavily.py` / `multpl.py` / `wgc.py` / `_common.py` | **API adapters** — cn-pivot 后主体为 akshare(含外围环境新方法 `get_us_treasury_10y`/`get_sp500_quote`/`get_fx_usdcny_realtime`/`get_cn_qvix`)+ Tavily(news/research)+ `multpl.py:fetch_multpl_series`(multpl.com 月度 HTML 表格爬虫,CAPE+美债10Y → ERP)+ `wgc.py:fetch_gold_aisc`(WGC fsapi 黄金 AISC 季度 JSON)。`yfinance.py`/`finnhub.py`/`alphavantage.py` 已删。`akshare.py` carries eastmoney throttling mitigations (UA/Referer, negative cache, backoff, `get_stock_quote`) |
...
| `market/gold/` | `provider.py` / `indicators.py` / `valuation.py` | `GoldMarketProvider` — 黄金 section(akshare SGE 价格 + FRED M2/CPI);`valuation.py:fetch_gold_valuation` + `render_gold_valuation_card`(TIPS 10年分位 + 金价 vs AISC 溢价 14年分位,任一缺则该行缺省);`render_section` 透传(gold 的 risk_html 已包含 `render_gold_section` + valuation card 输出) |
...
- **外围估值(ERP)**: multpl.com `/shiller-pe`(CAPE,即 Shiller PE)+ `/10-year-treasury-rate`(美债10Y 月度),由 `datasources/multpl.py:fetch_multpl_series` 爬 HTML 月度表格(移植自 golden-butterfly-dashboard)。`data/valuation_data.py:ValuationData.update_erp` 计算 `ERP = 1/CAPE×100 − 美债10Y`(百分点),三条月度序列(SHILLER_PE / US_10Y_BOND / ERP)落 `macro_data(country='us')`。由 `market/overseas.py` 渲染为外围卡 ERP cell。
- **黄金 AISC**:WGC fsapi `fsapi.gold.org/api/productioncosts/v11/charts/aisc`(匿名 JSON,Metals Focus 行业聚合,季度 2012-至今),由 `datasources/wgc.py:fetch_gold_aisc` 抓取,经 `data/gold_data.py` 落 `macro_data(GOLD_AISC/global)`。`market/gold/valuation.py:fetch_gold_valuation` + `render_gold_valuation_card` 渲染金价 vs AISC 溢价 card。
- **CN 红利低波100(930955)**:akshare `stock_zh_index_value_csindex(symbol='930955')`,取最新股息率,落 `macro_data(DIVIDEND_YIELD_930955/cn)`;由 `market/cn/provider.py:get_dividend_valuation` + `_render_dividend_valuation` 渲染红利低波100 估值信号 card(股息率 vs CN10Y 利差 + 信号阈值)。注:930904 = 中证 PPP 主题指数,**非** 红利低波;真正的「中证红利低波动指数」(50 只)= H30269,「红利低波100」(100 只)= 930955(本报告用 930955)。
...
- **数据源 5 路(akshare/FRED/Tavily/multpl/WGC),零 yfinance** — cn-pivot 后已删 `datasources/yfinance.py`/`finnhub.py`/`alphavantage.py`。新数据需求优先走 `datasources/akshare.py`(CN/外围/黄金),news/research 走 Tavily,US M2/CPI(gold indicator)走 FRED,ERP/CAPE 走 multpl,黄金 AISC 走 WGC fsapi。如需引入新数据源,加到 `datasources/` 并经 `data/` 层落库,不要绕过数据层直接在 provider 里 inline fetch。
- **估值类指标诚实性约定** — ERP / 红利低波 / AISC / TIPS 等估值类指标失败时:**DB-First 读最近值 + sub 标注时点**(`fetch_gold_valuation` / `get_dividend_valuation` / `fetch_overseas_data` 均按此实现);DB 空 + 源失败 → 该行/card 缺省,**绝不 fallback 静态常量冒充实时**(区别于 `FED_FUNDS_RATE` 这类政策常量 —— 后者本就是 FOMC step function,不冒充实时)。避免用陈旧或编造的估值数字误导投资决策。

...
- **CN 红利低波100(930955)**:akshare `stock_zh_index_value_csindex(symbol='930955')`,取最新股息率,落 `macro_data(DIVIDEND_YIELD_930955/cn)`;由 `market/cn/provider.py:get_dividend_valuation` + `_render_dividend_valuation_card` 渲染红利低波100 估值信号 card(股息率 vs CN10Y 利差 + 信号阈值)。注:930904 = 中证 PPP 主题指数,**非** 红利低波;真正的「中证红利低波动指数」(50 只)= H30269,「红利低波100」(100 只)= 930955(本报告用 930955)。

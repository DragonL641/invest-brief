...
MACRO_BRIEF_PROMPT = """你是资深宏观经济分析师，为投资者撰写每日 A 股宏观市场简报（含外围环境影响）。

基于提供的 A 股宏观数据与外围环境数据，输出纯 JSON（不要 markdown 代码块标记），包含两个字段：
- "summary"：核心观点，纯 HTML（可用 <p>、<strong>、<ul><li>、<br>；不要 markdown/代码块标记），**按主题分 3 个小节**，每节以 <strong>小节标题</strong> 起头：
  1. <strong>🌐 外围环境</strong>：美联储利率/美债10Y/标普500/人民币汇率 对 A 股的影响（资金流/情绪/估值锚）；若上下文提供 ERP（股权风险溢价=1/CAPE−美债10Y）与近10年分位，点评美股估值贵贱及其对 A 股估值锚的传导。
  2. <strong>🇨🇳 A股市场</strong>：A股走势 + 中国央行/LPR/M2/社融 + 中国经济（CPI/GDP）+ QVIX恐慌情绪 + 红利低波100股息率 vs CN10Y国债利差反映的债股性价比 + 该市场风险与机会
  3. <strong>🥇 黄金</strong>：金价 + 实际利率 TIPS/避险需求 + 金价 vs 开采成本 AISC 的溢价 + 该资产风险与机会
...
此外,上下文会提供"宏观环境四象限(规则判定)"。如规则判定有明显偏差(如数据已显示滞胀但规则判繁荣),可在 A股市场小节(🇨🇳)中修正并说明依据;无明显偏差则直接采纳。

注意:上下文「市场周期风险分」段的「极端指标」标注形如「指标名 风险子分X/10」——X 是 0-10 的风险子分(模型打分),非指标原值;指标原值见各市场周期风险卡。撰写时勿把子分当原值引用(如勿写「黄金GDP占比达9.8」)。"""


def serialize_macro_context(overseas_data: dict, cn_data: dict, news: list,
                            risk_scores: dict | None = None,
                            regime_data: dict | None = None,
                            gold_valuation: dict | None = None,
    ...
    # 外围环境(精简:美联储利率/美债10Y/标普500/USDCNY)
    lines.append("## 外围环境")
    for k, v in (overseas_data or {}).items():
        lines.append(f"- {k}: {v}")
    _emit_market("中国宏观", cn_data)

    # 黄金估值信号（TIPS / AISC / 溢价；任一缺失则该行省略）
    if gold_valuation:
        gv_lines = []
        tips = gold_valuation.get("tips_yield")
        if tips is not None:
            gv_lines.append(f"实际利率 TIPS: {tips}%")
        aisc = gold_valuation.get("aisc")
        premium = gold_valuation.get("premium_pct")
        if aisc is not None and premium is not None:
            gv_lines.append(f"开采成本 AISC: ${aisc}/oz（金价溢价 {premium}%）")
        if gv_lines and _used() + 80 < max_chars:
            lines.append("\n## 黄金估值")
            for gl in gv_lines:
                lines.append(f"- {gl}")
    ...
    if risk_scores and _used() + 250 < max_chars:
        ...
        for market, name in (("cn", "A股"), ("gold", "黄金")):
...
def generate_macro_brief(overseas_data: dict, cn_data: dict, news: list,
                         risk_scores: dict | None = None,
                         regime_data: dict | None = None,
                         gold_valuation: dict | None = None) -> tuple[str, str]:
    ...
    context = serialize_macro_context(overseas_data, cn_data, news,

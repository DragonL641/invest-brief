...
- 按三个小节组织，每节以 <strong>小节标题</strong> 起头：
  1. <strong>🇺🇸 美股</strong>
  2. <strong>🇨🇳 A股·中国</strong>
  3. <strong>🌍 全球其他</strong>
- 只列「有条目」的小节；某市场本周无条目，整节省略。
- 每个市场小节用 <ul><li> 分点陈列：每个 <li> 以 <strong>机构名</strong> 起头，接 1 句精炼观点（同一机构的多条合并为一条）。
- 不要把多家机构揉成一整段连续叙述；不同机构各占一条 <li>，可读性优先。
- 不要在观点末尾附 (来源域名, 日期) 等引用标注（机构名加粗已足够溯源，避免正文噪声）。
- 只用提供的数据，不编造观点、数字或机构。

"""P4 渲染管线：真实 DB → 两市场风险卡 → market_section_html 含全部卡片。"""
import os
...
def test_two_market_risk_cards_render():
    rc = {"color_up": "#e74c3c", "color_down": "#27ae60"}
    cn = CNMarketProvider(data=CNData())
    gold_data = GoldData()
    risk = {}
    for m, data_source in (("cn", cn.data), ("gold", gold_data)):
